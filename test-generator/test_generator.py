...
def convert_code_fragments_to_file(dataset_name: str, code_fragment: str, scope_variables: []):
    ...
    gen_file = open("temp_gen.py", "wt")
    gen_file.write(data)
    gen_file.close()

    # Auto Indentation the Python file
    cmd = "autopep8 /home/sanjelarun/prog_synth/prog_synth/test-generator/temp_gen.py " \
          "--in-place --pep8-passes 2000 --verbose"
    stream = os.popen(cmd)
    print(stream.read())
...
def generate_test_cases():
    ...
    cmd = "pynguin --project-path " + WORKING_DIR + " --output-path " + TEST_GEN_DIRECTORY + \
          " --module-name temp_gen -v"
    stream = os.popen(cmd)
    print()


def extract_only_test(generate_file_path="test-gen/generate_tests/test_temp-gen.py"):
    with open('/test-generator/temp_gen.py') as gen_in:
        allLines = gen_in.read()

    with open('/test-generator/test-gen/generate_tests/test_temp_gen.py') as fin:
        data = fin.read()
